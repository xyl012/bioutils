...
//! Traits to replace non-ACTG characters with pseudorandom bases.
//! # Examples
//! ```
//! 
//! extern crate rand;
...
// impl ReplaceNucleotide<String> for String {
    ...
    //     self.chars()
    //         .map(|ch| match ch {
    //             b'N' => *bases.choose(&mut rng).unwrap(),
    //             b'n' => *lowercase_bases.choose(&mut rng).unwrap(),
    //             _ => ch,
    //         })
    //         .collect()
    // }
...
fn test_replace_u_with_t() {
    ...
    let test = "ACTG".replace_u_with_t();
    assert_eq!(test, "ACUG");
}
}
