...
//     // fn is_type_passing_percent(&self, cutoff_value: &u8, cutoff_percent: &u8) -> Result<bool> {
...
//     // fn find_subseq(&self, sequence: &[u8]) -> Option<usize> {
//     //     self.windows(sequence.len()).position(|window| window == needle)
//     // }
...
//         if self.mut_quality_slice_passing_percent(quality_score)? >= (*percent).into() {
//             Ok(self)
//         } else { Ok(self) }
//     }
