...
//         // .for_each(|u| PHRED33_HASHMAP_U8.get(u).unwrap().to_owned()).collect::<Result<Vec<u8>>>()
//     }
//     // .into_iter().map(|(_, v)| v)
...
//     /// Returns the SANGER quality score from a SANGER quality encoding. The score is the u8 minus 33.
//     fn decode_qual_sanger(&self) -> Result<Vec<u8>> {
//         self.as_ref().iter().map(|q| SANGER_HASHMAP_DECODE_U8.get(&q)?.to_owned()).collect::<Result<Vec<u8>>>()
//     }
//     /// Returns the PHRED33 quality encoding from a PHRED33 quality score. The score is the u8 minus 33.
//     fn encode_qual(&self) -> Result<Vec<u8>> {
//         self.as_ref().iter().map(|q| PHRED33_HASHMAP_ENCODE_U8.get(&q)?.to_owned()).collect::<Result<Vec<u8>>>()
//     }
//     /// Returns the PHRED64 quality encoding from a PHRED64 quality score. The score is the u8 minus 64.
//     fn encode_qual_phred64(&self) -> Result<Vec<u8>> {
//         self.as_ref().iter().map(|q| PHRED64_HASHMAP_ENCODE_U8.get(&q)?.to_owned()).collect::<Result<Vec<u8>>>()
//     }
//     /// Returns the SANGER quality encoding from a SANGER quality score. The score is the u8 minus 33.
//     fn encode_qual_sanger(&self) -> Result<Vec<u8>> {
//         self.as_ref().iter().map(|q| SANGER_HASHMAP_ENCODE_U8.get(&q)?.to_owned()).collect::<Result<Vec<u8>>>()
//     }
// }
    ...
    T: AsRef<[u8]>,
{
    /// Returns the PHRED33 quality score from a raw PHRED33 quality encoding (-33).
    fn decode_qual(&self) -> Result<Vec<u8>> {
        self.as_ref().iter().map(|u| PHRED33_HASHMAP_DECODE_U8.get(u).copied()).collect::<Option<Vec<u8>>>().ok_or(bail!("Contains non-PHRED33 u8s"))
    }
    /// Returns the PHRED64 quality score from a raw PHRED64 quality encoding (-64).
    fn decode_qual_phred64(&self) -> Result<Vec<u8>> {
        self.as_ref().iter().map(|u| PHRED64_HASHMAP_DECODE_U8.get(u).copied()).collect::<Option<Vec<u8>>>().ok_or(bail!("Contains non-PHRED64 u8s"))
    }
    /// Returns the PHRED33 quality encoding from a PHRED33 quality score. (+33)
    fn encode_qual(&self) -> Result<Vec<u8>> {
        self.as_ref().iter().map(|u| PHRED33_HASHMAP_ENCODE_U8.get(u).copied()).collect::<Option<Vec<u8>>>().ok_or(bail!("Contains non-PHRED33 score u8s"))
    }
    /// Returns the PHRED64 quality encoding from a PHRED64 quality score. (+64)
    fn encode_qual_phred64(&self) -> Result<Vec<u8>> {
        self.as_ref().iter().map(|u| PHRED64_HASHMAP_ENCODE_U8.get(u).copied()).collect::<Option<Vec<u8>>>().ok_or(bail!("Contains non-PHRED64 score u8s"))
    }
}
