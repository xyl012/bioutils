...
fn main() {
    ...
    let mut qual = random_quality(50,rng); // Get a random quality vector of length 50, make it mutable so we can get our quality scores from the raw encoding
    ...
    let scores = qual.shift_phred33_qual_encoding().to_owned(); // get our quality scores from raw encoding and create an owned copy. We don't have to create an owned copy, this is just for demonstration.
